...
import { documentCycle } from './document-worker.mjs';
...
export class TeachingWorker {
 ...
 async cycle(){let {job}=await this.api('workspace-claim');if(!job)({job}=await this.api('claim'));if(!job)return this.config.documentEnabled===true ? documentCycle(this) : false;const resultRoute=(job.mode==='workspace'?'workspace/':'')+job.id+'/result';let heartbeatBusy=false;const timer=setInterval(async()=>{if(heartbeatBusy)return;heartbeatBusy=true;try{await this.api('heartbeat')}catch{}finally{heartbeatBusy=false}},20000);
