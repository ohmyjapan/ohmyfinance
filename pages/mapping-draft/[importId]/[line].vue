      ...
      <template v-else>
        ...
        <form @submit.prevent="save(false)">
            ...
            <section v-for="group in groups" :key="group.key" class="card mb-6 p-4 sm:p-6" :aria-labelledby="`group-${group.key}`">
              ...
              <div class="grid min-w-0 gap-6 sm:grid-cols-2">
                <DraftField v-for="field in fields.filter(f => f.group === group.key)" :key="field.key" :field="field" :evidence="draft.evidence[field.key]" :changed="!sameValue(values[field.key], draft.values[field.key])" :remembered="remember.includes(field.key)" :alternative-label="draft.evidence[field.key]?.state === 'conflict' && Object.hasOwn(draft.evidence[field.key], 'alternative') ? displayValue(field.key, draft.evidence[field.key].alternative) : ''" @alternative="applySuggestion({ field: field.key, value: draft.evidence[field.key].alternative, evidence: draft.evidence[field.key].alternativeEvidence })" :documents="draft.documents" :document-id="documentEvidence[field.key] || ''" :disabled="draft.locked" :class="field.kind === 'textarea' ? 'sm:col-span-2' : ''" @remember="rememberField(field.key, $event)" @document="documentEvidence[field.key] = $event">
                  ...
                  <input v-else :id="`draft-${field.key}`" v-model="values[field.key]" :type="field.kind === 'date' ? 'date' : 'text'" maxlength="1000" class="draft-control" :placeholder="field.key === 'referenceNumber' ? '空欄で自動採番' : field.key === 'invoiceNumber' ? 'T＋13桁の数字（注文番号とは別）' : ''">
          ...
          <section class="card mb-6 p-4 sm:p-6" aria-labelledby="documents-title">
            ...
            <DocumentList :documents="draft.documents" :values="values" readable :disabled="draft.locked || busy" :removable="!draft.locked && !dirty && !busy" @remove="removeDocument" @select="useDocument" @reading="updateDocument" />
            <div v-if="!draft.locked" class="mt-4 space-y-3"><div class="flex flex-wrap items-center gap-3"><select v-model="documentKind" :disabled="busy" class="draft-control w-auto" aria-label="添付する書類の種類"><option value="receipt">領収書</option><option value="invoice">請求書</option><option value="shipping">発送・顧客照合資料</option><option value="other">その他</option></select><label class="btn btn-secondary cursor-pointer text-sm" :class="dirty || !draft.revision || busy ? 'pointer-events-none opacity-50' : ''">書類を添付<input type="file" class="sr-only" accept="application/pdf,image/png,image/jpeg,image/webp" :disabled="dirty || !draft.revision || busy" @change="uploadDocument"></label></div><p class="text-xs text-gray-500 dark:text-gray-400">PDF・PNG・JPEG・WebP、各10MBまで。{{ dirty || !draft.revision ? '添付する前に下書きを保存してください。' : '添付後は内容の再確認が必要です。' }}添付した書類の「書類を読み取る」から候補を確認できます。発送シートはPDFで添付してください。</p></div>
          </section>
...
const documentSelections = ref<Record<string, any>>({})
const tagText = ref('')
...
const dirty = computed(() => !!draft.value && (!sameValue(values.value, draft.value.values) || accountingNote.value !== originalAccountingNote.value || !sameValue([...remember.value].sort(), [...draft.value.rememberedFields].sort()) || !sameValue(documentEvidence.value, originalDocuments.value) || Object.keys(documentSelections.value).length > 0))
...
const historyLabel = (action: string) => ({ customer_review: '顧客の商品代金の取引内容を確認', service_review: 'サービス利用料の候補を確認', chat_review: '会話で購入内容を確認', teaching_status: '学習ルールの適用を変更', slack_review: 'Slackで購入内容を確認', saved: '下書きを保存', approved: '内容を確認', document_added: '書類を添付', document_removed: '書類を削除' }[action] || action)
...
function receive(data: any) { documentSelections.value={}; draft.value = data; values.value = clone(data.values); tagText.value = data.values.tags.join(', '); remember.value = [...data.rememberedFields]; documentEvidence.value = clone(originalDocuments.value); accountingAnswer.value = data.accountingResponse ? clone(data.accountingResponse) : null; posting.value = ''; conflict.value = false }
...
async function save(confirm: boolean) { await run(async () => { receive(await $fetch(endpoint, { method: 'PUT', headers: user.authHeader, body: { ...identity(), values: values.value, remember: remember.value, confirm, documentEvidence: documentEvidence.value, documentSelections: documentSelections.value, accountingResponse: accountingNote.value.trim() ? {key:purchaseAssessment.value.key,note:accountingNote.value} : null } })); message.value = confirm ? '内容を確認して保存しました。まだ帳簿には登録されていません。' : '下書きを保存しました。' }) }
