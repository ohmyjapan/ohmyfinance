...
const historyLabel = (action: string) => ({ saved: '下書きを保存', approved: '内容を確認', document_added: '書類を添付', document_removed: '書類を削除' }[action] || action)
...
function applySuggestion(suggestion: any) { values.value[suggestion.field] = clone(suggestion.value); changedField(suggestion.field) }
