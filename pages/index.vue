<template>
  <div>
    ...
    <div class="mb-8">
          ...
          <div class="hidden md:block">
            <div class="flex items-center gap-4">
              <div class="text-right">
                ...
                <p class="text-3xl font-bold font-mono">{{ dashboardData?.stats?.total?.count || 0 }}</p>
              </div>
              <div class="w-px h-12 bg-white/30"></div>
              <div class="text-right">
                <p class="text-sm text-primary-light">{{ t('dashboard.upcomingPayments') }}</p>
                <p class="text-3xl font-bold font-mono">{{ dashboardData?.stats?.upcomingPaymentsCount || 0 }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="!isLoading && dashboardData && !dashboardData.hasOrganization" class="mt-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl p-4">
          ...
          <div class="flex-1">
            ...
            <div class="mt-3 flex gap-3">
              <NuxtLink
                ...
                class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-xl bg-amber-600 text-white hover:bg-amber-700 transition-colors"
              >
                {{ t('dashboard.createOrganization') }}
              </NuxtLink>
              <NuxtLink
                to="/settings/organization/join"
                class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-xl bg-white dark:bg-white/5 text-amber-600 dark:text-amber-400 border border-amber-300 dark:border-amber-600 hover:bg-amber-50 dark:hover:bg-white/[0.07] transition-colors"
              >
    ...
    <div class="rounded-2xl border bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 backdrop-blur-sm overflow-hidden mb-8">
      ...
      <div v-else-if="!recentTransactions || recentTransactions.length === 0" class="p-8 text-center text-gray-500">
        ...
        <NuxtLink
          to="/transactions/upload"
          class="mt-3 inline-flex items-center px-4 py-2 text-sm font-medium rounded-xl bg-primary-main text-white hover:bg-primary-dark transition-colors touch-manipulation"
        >
      ...
      <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-white/10">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-white/10">
          <tr
            ...
            :key="tx._id || tx.id"
            class="hover:bg-gray-50 dark:hover:bg-white/[0.07] cursor-pointer"
          ...
          >
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium font-mono" :class="tx.type === '入金' ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-gray-100'">
      ...
      <div v-if="recentTransactions && recentTransactions.length > 0" class="p-4 border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5">
        <div class="flex items-center justify-between">
          ...
          <NuxtLink
            to="/transactions"
            class="text-sm text-primary-main hover:text-primary-dark dark:text-primary-light"
          >
            {{ t('common.viewAll') }} →
          </NuxtLink>
        </div>
...
const getStatusClass = (status: string) => {
  const classes: Record<string, string> = {
    ...
    cancelled: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-white/5 dark:text-gray-400'
  }
