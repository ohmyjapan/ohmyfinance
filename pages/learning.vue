    ...
    <template v-else>
      ...
      <section v-if="overview.policies?.length" class="card mb-6 p-4 sm:p-6">
        ...
        <div v-for="policy in overview.policies" :key="policy._id" class="mt-4 flex flex-wrap items-start justify-between gap-3 border-l-2 border-primary-main/40 pl-3">
          <div class="min-w-0 flex-1"><p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ policy.title }}</p><p class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">{{ policy.reason }}</p><p class="mt-1 text-xs text-gray-500">{{ policy.status==='active'?'適用中':'停止中' }} · {{ policy.merchants.length }}種類の確認済み表記 · {{ policy.accountIds.length?'指定口座のみ':'全口座' }}</p><details class="mt-2 text-xs text-gray-500"><summary class="cursor-pointer text-primary-main">対象の利用先表記を見る</summary><p v-for="merchant in policy.merchants" :key="merchant" class="mt-1 break-words">{{ merchant }}</p></details></div>
...
async function togglePolicy(policy:any){saving.value=true;error.value='';try{await api('policies/'+policy._id,{method:'PUT',body:{revision:policy.revision,status:policy.status==='active'?'deferred':'active'}});overview.value=await api('overview');await loadPatterns(patterns.value.page);if(detail.value)await openPattern(detail.value.pattern._id)}catch(e:any){error.value=message(e)}finally{saving.value=false}}
