<template>
  <div class="learning">
    ...
    <template v-else>
      ...
      <section v-if="overview.policies?.length" class="card mb-6 p-4 sm:p-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100">判断に使う確認済みの指示</h2>
        <div v-for="policy in overview.policies" :key="policy._id" class="mt-4 flex flex-wrap items-start justify-between gap-3 border-l-2 border-primary-main/40 pl-3">
          <div class="min-w-0 flex-1"><p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ policy.title }}<span v-if="policy.kind==='teaching'" class="ml-2 text-xs font-normal text-primary-main">会話で記憶</span></p><p class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">{{ policy.reason }}</p><p class="mt-1 text-xs font-medium text-gray-700 dark:text-gray-300">{{ policy.decision.purpose==='company' ? '会社経費・顧客空欄' : overview.customers.find((c:any)=>c._id===policy.decision.customerId)?.name || '顧客購入' }}</p><p class="mt-1 text-xs text-gray-500">{{ policy.status==='active'?'適用中':'停止中' }} · {{ policy.merchants.length }}種類の確認済み表記 · {{ policy.accountName || (policy.accountIds.length?'指定口座のみ':'全口座') }}<span v-if="policy.effectiveFrom"> · {{ policy.effectiveFrom }}以降</span></p><NuxtLink v-if="policy.kind==='teaching'" :to="`/mapping-draft/${policy.importId}/${policy.line}`" class="mt-2 inline-block text-xs text-primary-main">元の会話と下書き ↗</NuxtLink><details class="mt-2 text-xs text-gray-500"><summary class="cursor-pointer text-primary-main">対象の利用先表記を見る</summary><p v-for="merchant in policy.merchants" :key="merchant" class="mt-1 break-words">{{ merchant }}</p></details></div>
          <button type="button" class="btn btn-secondary text-xs" :disabled="saving" @click="togglePolicy(policy)">{{ policy.status==='active'?'停止する':'再開する' }}</button>
        </div>
      </section>
        ...
        <div class="grid items-start gap-4" :class="detail?'lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]':''">
          <section class="card overflow-hidden" :class="detail?'hidden lg:block':''" aria-label="ルール案">
            ...
            <button v-for="p in patterns.items" :key="p._id" type="button" class="block w-full border-b border-gray-100 p-4 text-left transition-colors hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-main dark:border-white/10 dark:hover:bg-white/5" :class="detail?.pattern._id===p._id?'bg-primary-main/5':''" @click="openPattern(p._id)">
              <div class="flex items-start justify-between gap-3"><h2 class="min-w-0 break-words text-sm font-semibold text-gray-900 dark:text-gray-100">{{ p.merchantLabel }}</h2><span class="shrink-0 rounded-md px-2 py-1 text-xs font-medium" :class="gradeClass(p.answer?.grade==='A'?'A':p.grade)">{{ p.answer?.grade==='A'?'A · 確認済みの指示':p.answer?.grade==='B'?'B · 回答案あり':gradeText[p.grade] }}</span></div>
              ...
              <p v-if="p.answer?.values" class="mt-2 text-xs text-primary-main">OMFの回答：{{ p.answer.values.purpose==='company'?'会社経費':customerName(p.answer.values.customerId) }}</p>
          ...
          <section v-if="detail" class="card min-w-0 p-4 sm:p-6" aria-label="ルールの根拠と確認">
            ...
            <div v-if="detail.pattern.answer?.values" class="mt-5 rounded-xl bg-primary-main/5 p-3"><h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200">OMFの回答：{{ detail.pattern.answer.values.purpose==='company'?'会社経費':customerName(detail.pattern.answer.values.customerId) }}</h3><p class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">{{ detail.pattern.answer.reason }}</p><p class="mt-2 text-xs text-primary-main">{{ detail.pattern.answer.grade==='A'?'確認済みの指示から判断します。同じ内容を再入力する必要はありません。':'過去の傾向から候補を準備しました。下の内容を修正・確認できます。' }}</p></div>
            <p v-else-if="detail.pattern.answer?.conflict" class="mt-4 text-xs text-amber-700">{{ detail.pattern.answer.reason }}</p>
            <form class="mt-6 space-y-3 border-t border-gray-200 pt-5 dark:border-white/10" @submit.prevent="save('confirmed')">
              <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">回答を修正・記憶する</h3>
              <p class="text-xs leading-relaxed text-gray-500 dark:text-gray-400">確認した顧客・用途は、条件に合う新しい下書きに反映します。保存済みの修正は保持し、相違がある項目だけを確認します。</p>
...
async function openPattern(id:string) { const ticket=++detailId; busy.value=true; source.value=null; saveError.value=''; saved.value=false; try { const data=await api('patterns/'+id); if(ticket!==detailId)return; detail.value=data; const d=data.pattern.decision, answer=data.pattern.answer; Object.assign(form,{purpose:d?.purpose || answer?.values?.purpose || '',customerId:d?.customerId || answer?.values?.customerId || '',effectiveFrom:d?.effectiveFrom || answer?.effectiveFrom || new Date().toLocaleDateString('sv-SE',{timeZone:'Asia/Tokyo'}),note:d?.note || answer?.reason || ''}); await nextTick(); detailTitle.value?.focus() } catch(e:any) { error.value=message(e) } finally { if(ticket===detailId) busy.value=false } }
