...
import { learningOverview, learningPatterns, learningDetail, learningRows, learningRow, saveLearningDecision, savePolicyStatus, saveTeachingStatus } from '../../services/financeLearningService'
export default defineEventHandler(async event => {
  ...
  try {
    ...
    if (event.method==='PUT' && parts.length===2 && ['patterns','policies','lessons'].includes(parts[0])) {
      ...
      return parts[0]==='lessons' ? await saveTeachingStatus(ownerId,parts[1],body) : parts[0]==='policies' ? await savePolicyStatus(ownerId,parts[1],body) : await saveLearningDecision(ownerId,parts[1],body)
    }
