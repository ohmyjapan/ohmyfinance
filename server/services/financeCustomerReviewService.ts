...
export async function customerReviewPreview(ownerId:string,importIds:any){
 ...
 for(const d of drafts){const context=customerGoodsContext(d);if(!context)continue;const groupId=digest(customerReviewGroup(context));if(!groups.has(groupId))groups.set(groupId,{id:groupId,...context,workflow:contexts.filter((c:any)=>c.customerId===context.customerId),rows:[]});groups.get(groupId).rows.push({importId:d.importId,line:d.line,date:d.source.purchaseDate,amount:d.source.amount,mappingReason:context.mappingReason,originalCategory:context.originalCategory,choices:customerTreatments.map(t=>choice(d,t.id)).filter(Boolean)})}
