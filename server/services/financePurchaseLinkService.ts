...
export async function savePurchaseLink(ownerId:string,importId:string,line:number,body:any){
 ...
 await withPurchaseGraph(ownerId,graphCheck=>withWritableDraft(ownerId,importId,line,body,async(d,check)=>{
  ...
  await checkPurchaseAllocations(ownerId,old,candidate.order)
...
export async function releasePurchaseLink(ownerId:string,importId:string,line:number,body:any){
 ...
 await withPurchaseGraph(ownerId,graphCheck=>withWritableDraft(ownerId,importId,line,body,async(d,check)=>{
  const old=await Link.findOne({_id:id(body.id),ownerId,importId,line}).lean();await checkPurchaseAllocations(ownerId,old)
  await check();await graphCheck();const result=await Link.updateOne({_id:id(body.id),ownerId,importId,line,key:d.key,sourceHash:d.sourceHash,revision:body.linkRevision,status:'linked'},{$set:{status:'released'},$inc:{revision:1},$push:{history:{at:new Date(),action:'released'}}})
