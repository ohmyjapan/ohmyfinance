...
export const reportSchema={
 ...
 properties:{
  ...
  findings:{type:'array',items:{anyOf:[
   ...
   findingSchema(researchFields.filter(f=>f.key!=='purpose'&&f.kind!=='number').map(f=>f.key),{type:'string'})
  ]}},
...
const diagnosticMessages={question_repeated:'回答済みの内容を質問しているため、結果を保存できませんでした。',search_scope_invalid:'検索範囲を超える断定が残ったため、結果を保存できませんでした。',search_review_failed:'調査の説明・質問を確認できなかったため、結果を保存できませんでした。',purpose_invalid:'用途の選択肢が正しくないため、結果を保存できませんでした。',purpose_customer_conflict:'用途と顧客の組み合わせを確認できなかったため、結果を保存できませんでした。',output_invalid:'提案の項目または引用を検証できませんでした。',evidence_invalid:'取得した資料を検証できませんでした。',interpreter_failed:'AIの調査を完了できませんでした。',timed_out:'調査が制限時間内に完了しませんでした。',delivery_failed:'調査結果の送信または受理を完了できませんでした。'};
