...
export function customerGoodsContext(draft){
 ...
 const goods=['商品代金','상품대금','商品仕入'];
 if(!goods.includes(category?.name)||draft.source.category&&!goods.includes(draft.source.category)||assessPurchaseAccounting(draft).rule||draft.purchaseHistory?.status==='review')return null;
