...
export class ResearchWorker {
 ...
 async cycle(){
  ...
  const runtime=async()=>{const h=createHash('sha256');for(const file of ['./worker.mjs','./tools.mjs','./sheets.mjs','./sheet-export.mjs','./issey-archive.mjs','./search-evidence.mjs','./search-review.mjs','./accounting-guidance.mjs','./question-guidance.mjs','./evidence-repair.mjs','../shared/finance-accounting-assessment.mjs','./mcp.mjs','../shared/finance-research.mjs','../shared/finance-draft.mjs'])h.update(await fs.readFile(new URL(file,import.meta.url)));return {models:[...models],promptHash:createHash('sha256').update(JSON.stringify([researchSystem,correctionSystem,searchReviewSystem,scopeRepairSystem,evidenceRepairSystem])).digest('hex'),implementationHash:h.digest('hex'),durationMs:Date.now()-started}};
  try{
   ...
   const questions=questionGuidance(job.context);await fs.writeFile(path.join(dir,'question-guidance.json'),JSON.stringify(questions));
   const guidance=accountingGuidance(job.context);await fs.writeFile(path.join(dir,'accounting-guidance.json'),JSON.stringify(guidance));
   const output=await this.invokeInterpreter(this.config.cliPath,args,{cwd:this.config.inferenceDirectory,env},{instruction:job.instruction,context:job.context,accountingGuidance:guidance,questionGuidance:questions,capturedSources:job.sources},remaining(420000));
   ...
   const check=async(attempt)=>{
    ...
    if(!hasSearchEvidence(sources)&&!hasSettledQuestion(report,questions))return;
    stage='search_review_failed';
    const output=await this.invokeInterpreter(this.config.cliPath,noTools(searchReviewSchema(report,sources,questions),searchReviewSystem),{cwd:this.config.inferenceDirectory,env},{report:reportProse(report),context:job.context,questionGuidance:questions,capturedSources:sources},remaining(90000));
    ...
    const issues=validateSearchReview(checked.structured_output||JSON.parse(checked.result||'{}'),report,sources,questions);
    stage='output_invalid';if(issues.length){const error=searchScopeError(issues);if(issues.every(issue=>issue.sourceIds.some(id=>sources.find(s=>s.id===id)?.kind==='context')))error.code='question_repeated';throw error;}
   };
   try{await check(0)}catch(error){
    if(stage==='search_review_failed'||error.message==='Research timed out')throw error;
    correctionAttempted=true;const original=report,wordingOnly=['search_scope_invalid','question_repeated'].includes(error.code);
    const evidencePlan=error.code==='finding_evidence_invalid'?evidenceRepairPlan(original,job.context,sources):null;
    if(evidencePlan)await fs.writeFile(path.join(dir,'evidence-repair-plan.json'),JSON.stringify(evidencePlan));
    const schema=wordingOnly?scopeRepairSchema(original):evidencePlan?evidenceRepairSchema(original,evidencePlan):reportSchema;
    const prompt=wordingOnly?scopeRepairSystem:evidencePlan?evidenceRepairSystem:correctionSystem;
    const repaired=await this.invokeInterpreter(this.config.cliPath,noTools(schema,prompt),{cwd:this.config.inferenceDirectory,env},{validationFeedback:validationFeedback(error),...(evidencePlan?{evidenceIssues:evidencePlan.issues}:{}),report,context:job.context,questionGuidance:questions,...(wordingOnly||evidencePlan?{}:{accountingGuidance:guidance}),capturedSources:sources},remaining(90000));
    await fs.writeFile(path.join(dir,'repair.json'),String(repaired));const fixed=JSON.parse(repaired);for(const model of Object.keys(fixed.modelUsage||{}))models.add(model);if(fixed.is_error)throw Error('Research correction failed');const correction=fixed.structured_output||JSON.parse(fixed.result||'{}');report=wordingOnly?applyScopeRepair(original,correction):evidencePlan?applyEvidenceRepair(original,correction,evidencePlan,job.context,sources):correction;validateReport(report,job.context,sources);
    await check(1);
   }
   ...
   const diagnostic={code:e.message==='Research timed out'?'timed_out':['purpose_invalid','purpose_customer_conflict','search_scope_invalid','question_repeated'].includes(e.code)?e.code:stage,correctionAttempted};
