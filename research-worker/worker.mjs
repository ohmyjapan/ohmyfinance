...
export class ResearchWorker {
 ...
 async cycle(){
  ...
  const jobRoute=(job.mode==='evaluation'?'evaluation/':'')+job.id,started=Date.now();
  ...
  const runtime=async()=>{const h=createHash('sha256');for(const file of ['./worker.mjs','./tools.mjs','./mcp.mjs','../shared/finance-research.mjs','../shared/finance-draft.mjs'])h.update(await fs.readFile(new URL(file,import.meta.url)));return {models:[...models],promptHash:createHash('sha256').update(researchSystem).digest('hex'),implementationHash:h.digest('hex'),durationMs:Date.now()-started}};
  try{
   ...
   try{validateReport(report,job.context,sources)}catch(error){
    ...
    const repairArgs=['-p','--output-format','json','--json-schema',JSON.stringify(reportSchema),'--tools','','--strict-mcp-config','--mcp-config','{"mcpServers":{}}','--setting-sources','','--settings','{"disableAllHooks":true}','--no-session-persistence','--max-turns','3','--system-prompt',correctionSystem];
    const repaired=await invoke(this.config.cliPath,repairArgs,{cwd:this.config.inferenceDirectory,env},{validationFeedback:validationFeedback(error),report,context:job.context,capturedSources:sources},90000);
   ...
   const diagnostic={code:e.message==='Research timed out'?'timed_out':['purpose_invalid','purpose_customer_conflict'].includes(e.code)?e.code:stage,correctionAttempted};
