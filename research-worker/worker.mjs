 ...
 async cycle(){
  ...
  try{
   ...
   try{await check(0)}catch(error){
    ...
    const repaired=await this.invokeInterpreter(this.config.cliPath,noTools(wordingOnly?scopeRepairSchema(original):reportSchema,wordingOnly?scopeRepairSystem:correctionSystem),{cwd:this.config.inferenceDirectory,env},{validationFeedback:validationFeedback(error),report,context:job.context,capturedSources:sources},remaining(90000));
