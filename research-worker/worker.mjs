...
export class ResearchWorker {
 ...
 async cycle(){
  ...
  const runtime=async()=>{const h=createHash('sha256');for(const file of ['./worker.mjs','./tools.mjs','./sheets.mjs','./search-evidence.mjs','./search-review.mjs','./accounting-guidance.mjs','./question-guidance.mjs','../shared/finance-accounting-assessment.mjs','./mcp.mjs','../shared/finance-research.mjs','../shared/finance-draft.mjs'])h.update(await fs.readFile(new URL(file,import.meta.url)));return {models:[...models],promptHash:createHash('sha256').update(JSON.stringify([researchSystem,correctionSystem,searchReviewSystem,scopeRepairSystem])).digest('hex'),implementationHash:h.digest('hex'),durationMs:Date.now()-started}};
  try{
   ...
   const check=async(attempt)=>{
    ...
    const output=await this.invokeInterpreter(this.config.cliPath,noTools(searchReviewSchema(report,sources,questions),searchReviewSystem),{cwd:this.config.inferenceDirectory,env},{report:reportProse(report),context:job.context,questionGuidance:questions,capturedSources:sources},remaining(60000));
   ...
   try{await check(0)}catch(error){
    ...
    const repaired=await this.invokeInterpreter(this.config.cliPath,noTools(wordingOnly?scopeRepairSchema(original):reportSchema,wordingOnly?scopeRepairSystem:correctionSystem),{cwd:this.config.inferenceDirectory,env},{validationFeedback:validationFeedback(error),report,context:job.context,questionGuidance:questions,...(wordingOnly?{}:{accountingGuidance:guidance}),capturedSources:sources},remaining(90000));
    await fs.writeFile(path.join(dir,'repair.json'),String(repaired));const fixed=JSON.parse(repaired);for(const model of Object.keys(fixed.modelUsage||{}))models.add(model);if(fixed.is_error)throw Error('Research correction failed');const correction=fixed.structured_output||JSON.parse(fixed.result||'{}');report=wordingOnly?applyScopeRepair(original,correction):correction;validateReport(report,job.context,sources);
    await check(1);
   }
