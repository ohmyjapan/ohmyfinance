...
export class ResearchWorker {
 ...
 async cycle(){
  ...
  try{
   ...
   const output=await this.invokeInterpreter(this.config.cliPath,args,{cwd:this.config.inferenceDirectory,env},{instruction:job.instruction,context:job.context,accountingGuidance:guidance,capturedSources:job.sources},remaining(420000));
   ...
   try{await check(0)}catch(error){
    ...
    correctionAttempted=true;const original=report,wordingOnly=error.code==='search_scope_invalid';
    const repaired=await this.invokeInterpreter(this.config.cliPath,noTools(wordingOnly?scopeRepairSchema(original):reportSchema,wordingOnly?scopeRepairSystem:correctionSystem),{cwd:this.config.inferenceDirectory,env},{validationFeedback:validationFeedback(error),report,context:job.context,...(wordingOnly?{}:{accountingGuidance:guidance}),capturedSources:sources},remaining(90000));
